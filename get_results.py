...
def get_covmat():
    ...
    parser.add_argument('--input', type=str, default="samples_all.pkl")
    ...
    with open(args.input[:-4] + ".txt", "w") as fout:
        fout.write('Coverage Mean: %.4f | Coverage Median: %.4f | Match Mean: %.4f | Match Median: %.4f' % \
                        (covs.mean(), np.median(covs), mats.mean(), np.median(mats)))
    pool.close()
    pool.join()
