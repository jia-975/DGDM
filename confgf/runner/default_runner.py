...
class DefaultRunner(object):
    ...
    def generate_samples_from_testset(self, start, end, generator, num_repeat=None, out_path=None):
        ...
        if out_path is not None:
            with open(os.path.join(out_path, 'val_gen%s_s%de%depoch%dmin_sig%.3f.pkl' % (generator, start, end, self.config.test.epoch, self.config.test.gen.min_sigma)), "wb") as fout:
                pickle.dump(all_data_list, fout)
            print('save generated %s samples to %s done!' % (generator, out_path))
            print('the file is :', os.path.join(out_path, 'val_gen%s_s%de%depoch%dmin_sig%.3f.pkl' % (generator, start, end, self.config.test.epoch, self.config.test.gen.min_sigma)))
        print('pos generation[%d-%d] done  |  Time: %.5f' % (start, end, time() - generate_start))  

        return all_data_list

    def dg_generate_samples_from_testset(self, start, end, generator, num_repeat=None, out_path=None, dg_model=None, w_dg=0.5, seed=2021):
        ...
        if out_path is not None:
            with open(os.path.join(out_path, 'ip_%s_epoch%dmin_sig%.3f_dg_%.3f_seed%d.pkl' % (
            generator, self.config.test.epoch, self.config.test.gen.min_sigma, w_dg, seed,
                )), "wb") as fout:
                pickle.dump(all_data_list, fout)
            print('save generated %s samples to %s done!' % (generator, os.path.join(out_path, '%s_qm9_epoch%dmin_sig%.3f_dg_%.3f_%d_reversenums%d.pkl' % (
            generator, self.config.test.epoch, self.config.test.gen.min_sigma, w_dg, seed,self.config.test.gen.dg_num_steps))))
        print('pos generation[%d-%d] done  |  Time: %.5f' % (start, end, time() - generate_start))
    ...
    def dg_ConfGF_generator(self, data, config, dg_model, w_dg, pos_init=None):
        ...
        if pos_init is None:
            pos_init = torch.randn(data.num_nodes, 3).to(data.pos)
        data, pos_traj = self.dg_position_Langevin_Dynamics(dg_model, w_dg, data, pos_init, self._model, self._model.sigmas.data.clone(), \
                                            n_steps_each=config.steps_pos, step_lr=config.step_lr_pos, \
                                            clip=config.clip, min_sigma=config.min_sigma)
        pos_gen = pos_traj[-1, -1] #(num_node, 3) fetch the lastest pos
    ...
    def dg_position_Langevin_Dynamics(self, dg_model, w_dg, data, pos_init, scorenet, sigmas,
        ...
        for i, sigma in tqdm(enumerate(sigmas), total=sigmas.size(0), desc="Sampling positions"):
            ...
            for step in range(n_steps_each):
                ...
                dg_score = get_ratio(dg_model, data, d, batch_time)
