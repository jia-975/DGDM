...
class DualEncoderEpsNetwork(nn.Module):
    ...
    def __init__(self, config):
        ...
        """
        self.edge_encoder_global = get_edge_encoder(config) # 边的全局编码器
        self.edge_encoder_local = get_edge_encoder(config)  # 边的局部编码器
        ...
        model-global（边的全局特征、节点的全局特征、边的距离的梯度）
        '''
    ...
    def forward(self, atom_type, pos, bond_index, bond_type, batch, time_step, 
        ...
        if edge_index is None or edge_type is None or edge_length is None:
            ...
            edge_length = get_distance(pos, edge_index).unsqueeze(-1)   # (E, 1)获得每条边的距离
        local_edge_mask = is_local_edge(edge_type)  # (E, ) # 是否是本地边（除了虚拟边）
    ...
    def get_loss_diffusion(self, atom_type, pos, bond_index, bond_type, batch, num_nodes_per_graph, num_graphs, 
        ...
        pos_noise.normal_()
        pos_noise += 0.1 * torch.randn(pos_noise.shape) # todo: modify
    ...
    def langevin_dynamics_sample(self, atom_type, pos_init, bond_index, bond_type, batch, num_graphs, extend_order, extend_radius=True, 
                                 ...
                                 global_start_sigma=float('inf'), w_global=0.2, w_reg=1.0, w_ip=0.1, **kwargs):
        if self.model_type == 'diffusion':
            return self.langevin_dynamics_sample_diffusion(atom_type, pos_init, bond_index, bond_type, batch, num_graphs, extend_order, extend_radius, 
                        ...
                        sampling_type=kwargs.get("sampling_type", 'ddpm_noisy'), eta=kwargs.get("eta", 1.),w_ip=w_ip,)
        elif self.model_type == 'dsm':
    ...
    # todo
    def langevin_dynamics_sample_dg(self, dg_model, data, atom_type, pos_init, bond_index, bond_type, batch, num_graphs, extend_order, extend_radius=True,
                                 n_steps=100, step_lr=0.0000010, clip=1000, clip_local=None, clip_pos=None, min_sigma=0, is_sidechain=None,
                                 global_start_sigma=float('inf'), w_global=0.2, w_dg=0.1, w_reg=1.0, **kwargs):
        if self.model_type == 'diffusion':
            return self.langevin_dynamics_sample_diffusion_dg(dg_model, data, atom_type, pos_init, bond_index, bond_type, batch, num_graphs, extend_order, extend_radius,
                        n_steps, step_lr, clip, clip_local, clip_pos, min_sigma, is_sidechain,
                        global_start_sigma, w_global, w_dg, w_reg,
    ...
    # todo
    def langevin_dynamics_sample_diffusion_dg(self, dg_model, data, atom_type, pos_init, bond_index, bond_type, batch, num_graphs,
                                           extend_order, extend_radius=True,
                                           n_steps=100, step_lr=0.0000010, clip=1000, clip_local=None, clip_pos=None,
                                           min_sigma=0, is_sidechain=None,
                                           global_start_sigma=float('inf'), w_global=0.2, w_dg=0.1, w_reg=1.0, **kwargs):

        def compute_alpha(beta, t): # 计算alpha-bar
        ...
        with torch.no_grad():
            ...
            for i, j in tqdm(zip(reversed(seq), reversed(seq_next)), desc='sample'):
                ...
                if sampling_type == 'generalized' or sampling_type == 'ddpm_noisy':
                    ...
                    if sampling_type == 'generalized':  # DDIM
                    ...
                    elif sampling_type == 'ddpm_noisy':  # DDPM
                        atm1 = at_next
                        beta_t = 1 - at / atm1
                        e = -eps_pos
                ...
                elif sampling_type == 'ld':  # 朗之万动态采样
                    step_size = step_lr * (sigmas[i] / 0.01) ** 2
                    batch_time_step = torch.ones(batch.shape[0], device=pos.device) * i  # todo 乘以step_size or sigmas[i]
                    d = get_distance_from_pos(data, pos)
                    dg_judge = dg_model.judge(data, d, batch_time_step)  # 获得判别器结果 todo
                    pos_next = pos + step_size * (eps_pos + w_dg * dg_judge) / sigmas[i] + noise * torch.sqrt(step_size * 2)
    ...
    def langevin_dynamics_sample_diffusion(self, atom_type, pos_init, bond_index, bond_type, batch, num_graphs, extend_order, extend_radius=True, 
        ...
        sigmas = (1.0 - self.alphas).sqrt() / self.alphas.sqrt()    # （n_steps,)
        pos_traj = []
        if is_sidechain is not None:
            assert pos_gt is not None, 'need crd of backbone for sidechain prediction'
        with torch.no_grad():
            ...
            ## to test sampling with less intermediate diffusion steps
            # n_steps: the num of steps
            seq = range(self.num_timesteps-n_steps, self.num_timesteps)# （0， n_steps）
            seq_next = [-1] + list(seq[:-1])    # (-1, 0, 1, 2, ...)
            ...
            for i, j in tqdm(zip(reversed(seq), reversed(seq_next)), desc='sample'):
                ...
                noise = torch.randn_like(pos) + w_ip * torch.randn_like(pos)  #  center_pos(torch.randn_like(pos), batch)
                ...
                if sampling_type == 'generalized' or sampling_type == 'ddpm_noisy':
                    ...
                    step_size = step_lr * (sigmas[i] / 0.01) ** 2
                    pos_next = pos + step_size * eps_pos / sigmas[i]
...
@torch.no_grad()
def get_distance_from_pos(data, pos):
